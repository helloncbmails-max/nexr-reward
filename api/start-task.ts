import { createClient } from "@supabase/supabase-js";
import crypto from "crypto";

function verifyTelegramInitData(
  initData: string,
  botToken: string
) {
  const params = new URLSearchParams(initData);
  const receivedHash = params.get("hash");

  if (!receivedHash) {
    throw new Error("Invalid Telegram authentication data");
  }

  params.delete("hash");

  const dataCheckString = Array.from(params.entries())
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([key, value]) => `${key}=${value}`)
    .join("\n");

  const secretKey = crypto
    .createHmac("sha256", "WebAppData")
    .update(botToken)
    .digest();

  const calculatedHash = crypto
    .createHmac("sha256", secretKey)
    .update(dataCheckString)
    .digest("hex");

  const expectedBuffer = Buffer.from(
    calculatedHash,
    "hex"
  );

  const receivedBuffer = Buffer.from(
    receivedHash,
    "hex"
  );

  if (
    expectedBuffer.length !== receivedBuffer.length ||
    !crypto.timingSafeEqual(
      expectedBuffer,
      receivedBuffer
    )
  ) {
    throw new Error(
      "Telegram authentication failed"
    );
  }

  const authDate = Number(
    params.get("auth_date")
  );

  if (!authDate) {
    throw new Error(
      "Telegram authentication date missing"
    );
  }

  const now = Math.floor(Date.now() / 1000);
  const age = now - authDate;

  if (age < -60 || age > 86400) {
    throw new Error(
      "Telegram authentication data expired"
    );
  }

  const userData = params.get("user");

  if (!userData) {
    throw new Error(
      "Telegram user data missing"
    );
  }

  return JSON.parse(userData);
}

export default async function handler(
  req: any,
  res: any
) {
  if (req.method !== "POST") {
    return res.status(405).json({
      error: "Method not allowed",
    });
  }

  try {
    const {
      initData,
      task_id,
    } = req.body;

    if (!initData) {
      return res.status(400).json({
        error:
          "Missing Telegram authentication data",
      });
    }

    if (
      !task_id ||
      typeof task_id !== "string"
    ) {
      return res.status(400).json({
        error: "Missing task ID",
      });
    }

    const botToken =
      process.env.TELEGRAM_BOT_TOKEN;

    const supabaseUrl =
      process.env.SUPABASE_URL;

    const serviceRoleKey =
      process.env.SUPABASE_SERVICE_ROLE_KEY;

    if (
      !botToken ||
      !supabaseUrl ||
      !serviceRoleKey
    ) {
      console.error(
        "Missing server configuration"
      );

      return res.status(500).json({
        error:
          "Server configuration error",
      });
    }

    // Verify Telegram identity
    const telegramUser =
      verifyTelegramInitData(
        initData,
        botToken
      );

    const supabase = createClient(
      supabaseUrl,
      serviceRoleKey,
      {
        auth: {
          persistSession: false,
        },
      }
    );

    // Confirm Nexr user exists
    const {
      data: user,
      error: userError,
    } = await supabase
      .from("nexr_users")
      .select("id")
      .eq(
        "telegram_id",
        Number(telegramUser.id)
      )
      .single();

    if (userError || !user) {
      console.error(
        "Nexr user lookup error:",
        userError
      );

      return res.status(404).json({
        error:
          "Nexr user not found",
      });
    }

    // Confirm task exists and is active
    const {
      data: task,
      error: taskError,
    } = await supabase
      .from("tasks")
      .select(
        "id, title, description, reward, status"
      )
      .eq("id", task_id)
      .eq("status", "active")
      .single();

    if (taskError || !task) {
      console.error(
        "Task lookup error:",
        taskError
      );

      return res.status(404).json({
        error:
          "Task not found or inactive",
      });
    }

    // Check whether this user already has a completion
    const {
      data: existingCompletion,
      error: existingError,
    } = await supabase
      .from("task_completions")
      .select(
        "id, status, created_at"
      )
      .eq("user_id", user.id)
      .eq("task_id", task.id)
      .maybeSingle();

    if (existingError) {
      console.error(
        "Task completion lookup error:",
        existingError
      );

      return res.status(500).json({
        error:
          "Unable to check task status",
      });
    }

    if (existingCompletion) {
      return res.status(200).json({
        success: true,
        already_started: true,
        completion: existingCompletion,
        task,
      });
    }

    // Create a pending task completion
    const {
      data: completion,
      error: completionError,
    } = await supabase
      .from("task_completions")
      .insert({
        user_id: user.id,
        task_id: task.id,
        status: "pending",
      })
      .select(
        "id, user_id, task_id, status, created_at"
      )
      .single();

    if (completionError) {
      console.error(
        "Task completion creation failed:",
        completionError
      );

      return res.status(500).json({
        error:
          "Unable to start task",
      });
    }

    console.log(
      "Task started:",
      completion
    );

    return res.status(200).json({
      success: true,
      already_started: false,
      completion,
      task,
    });
  } catch (error) {
    console.error(
      "Start task error:",
      error
    );

    return res.status(401).json({
      error:
        error instanceof Error
          ? error.message
          : "Authentication failed",
    });
  }
}